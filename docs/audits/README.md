...
- Files:
  - `00-executive-summary.md` — verdict first: an H1 stating the verdict, then
    `- **Lens:**`, `- **Commit:**` and `- **Date:**` lines (these feed the
    index), then the one-paragraph judgement, scope, method, findings count by
    severity, top risks. Readable standalone by someone who opens nothing else.
